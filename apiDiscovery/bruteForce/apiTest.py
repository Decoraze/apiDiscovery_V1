...
def clearLogs():
    try:
        with open("Wordlists/acceptedLinks.txt","r") as f:
            data = f.readlines()

        with open("Wordlists/acceptedLinks.txt","w") as f:
            for line in data:
                f.write('')
...
def apiDetailsFromAcceptedLinks(lst):
    try:
        fileSize = os.stat("Wordlists/acceptedLinks.txt").st_size
    except FileNotFoundError:
        print("FileNotFound! Please check that you have the file: accecptedLinks and acceptedLinksOutput!")
        sys.exit()
    ...
    while fileSize != 0:
        links = getWordlist("Wordlists/acceptedLinks.txt")
        wl = getWordlist("Wordlists/acceptedLinksOutput.txt")
        for x in range (len(links)):
            l = links[x]
            if l != '' and ((l[-11::]) != "favicon.ico" and (l[-2::]) != "//"):
                for ep in lst:
                    url = str(l)+"/"+str(ep)
                    print(url)
                    ...
                    if output != None: 
                        with open("Wordlists/output.txt","a") as f:
...
def apiDetails(link):
        ...
        if response.reason == "OK":
            ...
            try:
                ...
                if json == {} or json == [] or json == ():
                    ...
                    with open("Wordlists/acceptedLinks.txt","a") as f:
                        f.write(str(response.url)+"\n")
                    aLO = getWordlist("Wordlists/acceptedLinksOutput.txt")
                    
                    if (str(response.url)) not in aLO:
                        with open("Wordlists/acceptedLinksOutput.txt","a") as f:
                            f.write(str(response.url)+"\n")
    ...
    except NameError or KeyboardInterrupt:
        print ("Error! Program has stopped unexpectedly Please check the link. (IMGs can cause this error)")
        sys.exit()
...
def main():
    ...
    flag = True
    try:
        usrInput = (input("Enter a command (type -h for help): ")).lower()
        while flag == True:
            if usrInput == "-h":
                print("\nThis is the ApiDiscovery Tool (BruteForce) Help Page!\n"
                      "----------------------------------------------------------------------------------------------\n"
...
def apiTest():
    ...
    try:
        ...
        print(result," Timeout. Execution Reached its Limit of 1 hour. Check the code to change the default value. Default value: 3600")

    except:

        print(" Timeout. The Program Execution Reached its Limit of 1 hour or has stopped unexpectedly. Check the code to change the default value. Default value: 3600")
    
    return "API BRUTE FORCE TEST"
